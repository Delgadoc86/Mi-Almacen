...
### v1.5.6 — 2026-07-20
- **Un solo listener de Firestore por colección, no uno por pantalla** — corrección interna de consumo de datos, sin cambios visibles para el usuario. Antes, `useProducts`/`useCustomers`/`useCashSession`/`useCategories` abrían su propio `onSnapshot` en cada pantalla que los usaba: visitar Inicio + Caja + Fiados + Productos + Lista de precios/PDF en la misma sesión releía las mismas colecciones 2-3 veces contra el servidor (sin caché offline configurada). Ahora un `BusinessDataProvider` nuevo (`src/context/BusinessDataContext.tsx`), montado una sola vez en `app/(app)/_layout.tsx`, mantiene un único listener por colección para todo el negocio — mismo patrón que `AuthContext` ya usaba para `business`. Los 4 hooks devuelven exactamente el mismo shape de siempre (`{ products, loading, error, retry }`, etc.), así que ninguna pantalla cambió. Reduce el consumo de lecturas de Firestore por sesión sin tocar Rules, Cloud Functions ni la lógica de negocio.

### v1.5.5 — 2026-07-17
- **Eliminación definitiva de cuenta desde el panel admin** — cierra el flujo que dejó abierto Fase 6/9: hasta ahora, cuando un cliente solicitaba eliminar su cuenta (`deletionRequest`), el borrado real seguía siendo 100% manual por soporte (edición directa en Firebase Console). Ahora el admin ve la solicitud en el dashboard, la lista de negocios (ícono de aviso) y el detalle del negocio, y ejecuta el borrado desde una "ZONA DE ELIMINACIÓN" propia: primero una revisión con conteos de lo que se va a borrar (productos, categorías, clientes, movimientos de fiado, sesiones y movimientos de caja, pagos de la libreta de cobros, si existe cuenta de Firebase Auth), y después una confirmación fuerte escribiendo el email exacto del dueño — sin eso, no se borra nada. Dos Cloud Functions nuevas (`adminGetDeletionPreview`, `adminDeleteRequestedAccount`), reintentable de forma segura si una ejecución queda a mitad de camino, con auditoría en `adminAuditLogs` que nunca guarda nombre, email ni montos — solo conteos. `firestore.rules` no cambió: el cliente ya no podía borrar nada directo, y estas funciones usan el Admin SDK. Desplegado 2026-07-20. Ver `docs/SAAS_ROADMAP.md`, Fase 10, y `DECISIONES_TECNICAS.md`, Fase 18.
...
- **Libreta de cobros administrativa** (`adminBilling/{businessId}`) — último pago, próximo cobro esperado, método, monto y notas internas, gestionada solo desde el panel admin vía 3 Cloud Functions nuevas. Puramente administrativo: no participa en `canWrite` ni en Rules, no activa ni corta nada automáticamente. Ver "Estado actual (SaaS)" arriba y `docs/SAAS_ROADMAP.md`, Fase 8. Desplegado 2026-07-20.
